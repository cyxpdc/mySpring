...
import com.pdc.resthandlers.WebClientRestHandler;
...
public class JDKProxyCreator implements ProxyCreator {
	...
	public Object createProxy(Class<?> type) {
		log.info("到达createProxy方法:" + type);//测试类为IUserApi
		// 根据接口得到API服务器信息，即url
		ServerInfo serverInfo = extractServerInfo(type);
		log.info("创建serverInfo:" + serverInfo);
		// 给每一个代理类一个实例
		RestHandler handler = new WebClientRestHandler();
		// 初始化服务器信息(和初始化webclient)
		handler.init(serverInfo);
		//创建接口代理类
		return newProxyInstance(type, handler);
	}
	 ...
	 * @return
	 */
	private Object newProxyInstance(Class<?> type, RestHandler handler) {
		return Proxy.newProxyInstance(this.getClass().getClassLoader(),
				new Class[] { type }, new InvocationHandler() {
					...
					public Object invoke(Object proxy, Method method,
						...
						log.info("创建methodInfo:" + methodInfo);
						// 调用rest
